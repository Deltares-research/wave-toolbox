...
def convert_to_array_type(x):
    ...
    if isinstance(x,numbers.Number):
       x = np.asarray([x])  # for some reason in this case the statement x = np.asarray(x) is not sufficient. 
...
def rfwave(h=None,T=None,H=None):                  
    ...
    xLimit=[50,   25.61,     25.6,   20.8,   17.6,   15.3,   13.5,   12.2,   11.1,   9.53,   8.39,   7.55,   6.89,   6.37,   5.59,   5.05,   4.64,   4.33,   4.08,   3.62,   0]
    yLimit=[0,      0,      0.786,  0.775,  0.765,  0.756,  0.747,  0.737,  0.728,  0.709,  0.689,  0.668,  0.646,  0.623,  0.576,  0.53,   0.487,  0.447,  0.412,  0.342,  0]
    ...
    if d<0:
        ...
        print('The wave characteristics fall outside the range of Rfwave. Only the results of linear wave theory are shown. Be cautious with the results! Is your input correct?') 
...
def rfwave_rfw(T=None, t=None, H=None, h=None, xeta=None, xvel=None, zvel=None):
    ...
    for tt in np.arange(0,len(t)):   # matlab tt = 1:length(t)
        eta[:,tt] = rfwave_rfeta(N, h, ceta, k, xeta/h, c, t[tt]*(g/h)**(1/2))
        [u[:,:,tt], w[:,:,tt] ]= rfwave_rfvel(B0,B,c,k,xvel/h,zvel/h,t[tt]*(g/h)**(1/2))
...
def rfwave_rfcoef(H=None, tau=None, N=None):
    ...
    while (H<=Hin):
          ...
          while (max(z)>1e-12 and numiter<100):
                ...
                f = np.concatenate((f, 0.5*u**2 + 0.5*v**2 + eta - R), axis=None)                      #  [f; 0.5*u.^2 + 0.5*v.^2 + eta - R]
                f = np.concatenate((f, 1/(2*N)*(eta[0] + eta[N] + 2*sum(eta[1:N+1])) - 1),axis=None)   #  [f; 1/(2*N)*(eta(1) + eta(N+1) + 2*sum(eta(2:N))) - 1];
                f = np.concatenate((f, eta[0] - eta[N] - H),axis=None)                                 #  [f; eta(1) - eta(N+1) - H];
                f = np.concatenate((f, k*c*tau - 2*np.pi),axis=None)                                   #  [f; k*c*tau - 2*pi];
                ...
                S1      = np.sinh(eta*jj*k)/np.cosh(np.ones((N+1,1))*jj*k*D)*np.cos(mm*jj*np.pi/N)
                dfidBj  = S1
                dfidc   = np.zeros((N+1,1))
                dfidk   = eta*(u-B0)/k - D*sum(np.ones((N+1,1))*(jj*B*np.tanh(jj*k*D))*S1,2)
                dfidQ   = np.ones((N+1,1))
                dfidR   = np.zeros((N+1,1))
                A       = np.concatenate((dfideta, dfidB0, dfidBj, dfidc, dfidk, dfidQ, dfidR),axis=None)   #[dfideta dfidB0 dfidBj dfidc dfidk dfidQ dfidR];
          
                #     For i = N+2:2N+2
                C1      = np.cosh(eta*jj*k)/np.cosh(np.ones((N+1,1))*jj*k*D)*np.sin(mm*jj*np.pi/N)
                C2      = np.cosh(eta*jj*k)/np.cosh(np.ones((N+1,1))*jj*k*D)*np.cos(mm*jj*np.pi/N)
                S2      = np.sinh(eta*jj*k)/np.cosh(np.ones((N+1,1))*jj*k*D)*np.sin(mm*jj*np.pi/N)
                dfideta = (np.diag(1 + u*k**2*sum(np.ones((N+1,1))*(jj**2*B)*S1,2)  
                ...
                dfidk   =  u*((u-B0)/k + k*eta*sum(np.ones((N+1,1))*(jj**2*B)*S1,2) - k*D*sum(np.ones((N+1,1))*(jj**2*B*np.tanh(jj*k*D))*C2,2)) \
                         + v*(v/k + k*eta*sum(np.ones((N+1,1))*(jj**2*B)*C1,2) - k*D*sum(np.ones((N+1,1))*(jj**2*B*np.tanh(jj*k*D))*S2,2))
                          
                             
                          
                dfidQ   = np.zeros((N+1,1),dtype=float)
                dfidR   = -np.ones((N+1,1))
                A       = np.concatenate((A, dfideta, dfidB0, dfidBj, dfidc, dfidk, dfidQ, dfidR),axis=None)    #[A; dfideta dfidB0 dfidBj dfidc dfidk dfidQ dfidR];
                #     For i = 2N+3
                dfideta = np.concatenate((1/(2*N),np.ones((1,N-1))/N, 1/(2*N)),axis=None)      #[1/(2*N) ones(1,N-1)/N 1/(2*N)];
                A       = np.concatenate((A, dfideta,np.zeros((1, N+5))),axis=None)               #[A; dfideta zeros(1, N+5)];
                #     For i = 2N+4
                dfideta = np.zeros((1,N+1)); dfideta[0] = 1; dfideta[N] = -1;
                A       = np.concatenate((A, dfideta, np.zeros((1, N+5))),axis=None)                 #[A; dfideta zeros(1, N+5)];
                #     For i = 2N+5
                dfidc   = k*tau
                dfidk   = c*tau
                A       = np.concatenate((A, np.zeros((1,2*N+2)), dfidc, dfidk, 0, 0),axis=None)   #[A; zeros(1,2*N+2) dfidc dfidk 0 0];
                #     For i = 2N+6
                dfidc   = 1
                dfidB0  = 1
                A       = np.concatenate((A, np.zeros((1,N+1)), dfidB0, np.zeros((1,N)), dfidc, 0, 0, 0),axis=None)      #[A; zeros(1,N+1) dfidB0 zeros(1,N) dfidc 0 0 0];
...
def rfwave_rfeta(N=None, h=None, ceta=None, k=None, x=None, c=None, t=None):
    ...
    Y   = 2/N*(0.5*ceta[0] + 
          sum(ceta[0:Nceta-3]*np.ones((1,N))*np.cos(mm[0:Nceta-3]*jj*np.pi/N),1) + 
          0.5*ceta[-1]*np.cos(jj*np.pi))
    eta = (sum(np.ones((len(x),1))*Y[0:Nceta-3]*np.cos((x-c*t)*jj[0:Nceta-3]*k),2) + 
          0.5*Y[-1]*np.cos(jj[-1]*k*(x-c*t)))/(k*h) + 1
...
def rfwave_rfvel(B0=None,B=None,c=None,k=None,x=None,z=None,t=None):
    ...
    UJ = JJ*BB*np.cosh(Z*JJ*k)/np.cosh(JJ*k*D)*np.cos((X-x0-c*t)*JJ*k)    #JJ.*BB.*cosh(Z.*JJ*k)./cosh(JJ*k*D).*cos((X-x0-c*t).*JJ*k);
    u  = c + B0 + k*sum(UJ,3)
    UJ =[]  #clear UJ;

    # Determine the vertical velocity v.
    WJ = JJ*BB*np.sinh(Z*JJ*k)/np.cosh(JJ*k*D)*np.sin((X-x0-c*t)*JJ*k)      #JJ.*BB.*sinh(Z.*JJ*k)./cosh(JJ*k*D).*sin((X-x0-c*t).*JJ*k);
    w  = k*sum(WJ,3)
...
def str2num_if_numeric(value:str=None)-> [(int or float or complex),bool]:
    ...
    if not isinstance(value, str) or isinstance(value, float) or isinstance(value, int) or isinstance(value, complex):
        number = value
        is_converted = False
...
def interp_measurement_timeseries(in_time: list or np.ndarray, in_signal: list or np.ndarray, new_time: list or np.ndarray):
    """
    ...
    if not isinstance(in_time, (list, np.ndarray)) or not isinstance(in_signal, (list, np.ndarray)) or not isinstance(new_time, (list, np.ndarray)):
