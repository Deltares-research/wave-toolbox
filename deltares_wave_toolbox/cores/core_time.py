...
def sort_wave_params(hWave=None,tWave=None):
    ...
    is1d_array = engine_core.is1darray(hWave)               # matlabmax( abs( _size(hWave) - size(tWave) ) ) > 0
    if (not is1d_array ):
       raise ValueError('sort_wave_params: Input error, Input arrays hWave is not 1D')

    is1d_array = engine_core.is1darray(tWave)              # matlabmax( abs( _size(hWave) - size(tWave) ) ) > 0
    if (not is1d_array ):
    ...
    if (not isequal_length ):
       raise ValueError('WaveLab:InputError','Input arrays hWave and tWave have different length')
    ...
    iSorted     = np.argsort(hWave)[::-1]   # term [::-1] reverses the array, meaning descending order.
...
def determine_zero_crossing(t=None,x=None,typeCross='down'):
    ...
    is1d_array = engine_core.is1darray(t)               # matlabmax( abs( _size(hWave) - size(tWave) ) ) > 0
    if (not is1d_array ):
       raise ValueError('sort_wave_params: Input error: Input arrays t is not 1D')

    is1d_array = engine_core.is1darray(x)               # matlabmax( abs( _size(hWave) - size(tWave) ) ) > 0
    if (not is1d_array ):
    ...
    if (not isequal_length ):
       raise ValueError('sort_wave_params: Input error: Input arrays t and x have different length')
    ...
    if ( typeCross.lower()=='up' ):
     ...
     for iTime in np.arange(0,nTime-1):          # matlab iTime = 1:(nTime-1)
        ...
        raise ValueError('sort_wave_params: Input error: Wrong input argument for typeCross ');
...
def highest_waves_params(hWaveSorted=None,tWaveSorted=None,fracP=None) -> [float,float]:
    """
    ...
    if (not is1d_array ):
       raise ValueError('highest_waves_params: Input error: Input arrays hWaveSorted is not 1D')
   
    is1d_array = engine_core.is1darray(tWaveSorted)              
    if (not is1d_array ):
       raise ValueError('highest_waves_params: Input error: Input arrays tWaveSorted is not 1D')

    # --- Check whether the length of arrays t and x the same
    isequal_length =(len(tWaveSorted) == len(hWaveSorted))
    if (not isequal_length ):
       raise ValueError('highest_waves_params: Input error: Input arrays tWaveSorted and hWaveSorted have different length')
    
 
    # --- Input parameter fracP should be between 0 and 1
    if ( fracP < 0 or fracP > 1 ):
       raise ValueError('highest_waves_params: Input error: Input parameter fracP should be between 0 and 1 ')
    ...
    issorted = (hWaveSorted == np.sort(hWaveSorted)[::-1]).all() # matlab if ( ~issorted( hWaveSorted(end:-1:1)) )
    if (not issorted):
       raise ValueError('highest_waves_params: Input error:Input array hWaveSorted is not correctly sorted ');

    #Computational core
    # --- Number of waves in the fraction that is to be considered
    nWaveP = int(np.floor( nWave * fracP )) # note hWaveSorted[0:1] is equal to hWaveSorted[0] (hWaveSorted[0:0])
    if nWaveP >0:
       hFracP = determine_mean( hWaveSorted[0:nWaveP] )  #  0:nWaveP means nWaveP elements, starting at index zero # matlab hWaveSorted(1:nWaveP) 
       tFracP = determine_mean( tWaveSorted[0:nWaveP] )  # see statement above, matlab tWaveSorted(1:nWaveP)
...
def exceedance_wave_height(hWaveSorted=None,excPerc=None)->float:
    ...
    is1d_array = engine_core.is1darray(hWaveSorted)              
    if (not is1d_array ):
       raise ValueError('exceedance_wave_height: Input error: Input arrays hWaveSorted is not 1D')
   
    #Check on input arguments
    # --- Input parameter excPerc should not exceed 100 (100%) or be smaller than 0
    isvalid_range = ( excPerc > 100 or excPerc < 0 )
    if (isvalid_range): 
       raise ValueError('exceedance_wave_height: Input error: Input parameter excPerc should be between 0 and 100 ');
    ...
    issorted = (hWaveSorted == np.sort(hWaveSorted)[::-1]).all()   # matlab if ( ~issorted( hWaveSorted(end:-1:1)) )
    if (not issorted):
       raise ValueError('exceedance_wave_height: Input error:Input array hWaveSorted is not correctly sorted ');

    # Computational core
    # --- Index of the wave corresponding to the given exceedance probability percentage
    iWaveP = int(np.floor( nWave * excPerc / 100.0 )) -1 # note here you select the actual value and not the range (e.g. hWaveSorted[ 1: 4]), so substract 1 as array index start at 0 instead of 1 (MATLAB)    
...
def determine_params_individual_waves(tCross=None,t=None,x=None):
    ...
    if (not is1d_array ):
       raise ValueError('determine_params_individual_waves: Input error: Input arrays tCross is not 1D')

    is1d_array = engine_core.is1darray(t)              
    if (not is1d_array ):
       raise ValueError('determine_params_individual_waves: Input error: Input arrays t is not 1D')
    
    is1d_array = engine_core.is1darray(x)              
    if (not is1d_array ):
       raise ValueError('determine_params_individual_waves: Input error: Input arrays x is not 1D')

    # --- Check whether the length of arrays t and x the same
    isequal_length = (len(t)==len(x))
    if (not isequal_length ): 
       raise ValueError('determine_params_individual_waves: Length of input arrays t and x not identical ');
    ...
    tWave   = np.zeros((1,nWave),dtype=float)[0]  # statement creates [[]], so use [0] to select array# matlab size(tCross(1:end-1))
    ...
    for iWave in np.arange(0,nWave):             # matlab iWave = 1:nWave
        ...
        iIni = np.where(t >= tIni)[0][0]       #find( t >= tIni, 1, 'first' ); # Position initial time
        iEnd = np.where(t <= tEnd)[0][-1]      #find( t <= tEnd, 1, 'last'  ); # Position end time
        ...
        iMax =  np.where( xIwave == max( xIwave ))[0][0] # matlab find( xIwave == max( xIwave ), 1, 'first' );
        iMin =  np.where( xIwave == min( xIwave ))[0][0]  # matlab find( xIwave == min( xIwave ), 1, 'first' );
